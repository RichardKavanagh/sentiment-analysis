...
public class TextPreProcessorBolt extends BaseBasicBolt {
	...
	public void execute(Tuple input, BasicOutputCollector collector) {
		...
		if (!message.contains(NULL_TEXT) || tweet.isRetweet()) {
			collector.emit(new Values(tweet, id, user, message, hashtags));
		}
		else if(tweet.isRetweet()) {
			LOGGER.info("Dropping Tweet: Id " + id + " as it is a retweet.");
		}
		else if(message.contains(NULL_TEXT)) {
			LOGGER.info("Dropping Tweet: Id " + id + " as it contains null text." + message);
		}
	}
